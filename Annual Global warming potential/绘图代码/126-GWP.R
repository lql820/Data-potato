...
plot_yield_trends <- function(region, nm_file, tm_file) {
  ...
  # 提取所需列
  df_nm <- df_nm %>% select(Year, mean_yield = Mean_GWP, 
                            sd_yield = SD_GWP)
  df_tm <- df_tm %>% select(Year, mean_yield = Mean_GWP, 
                            sd_yield = SD_GWP)
  ...
  plot <- ggplot() +
   ...
   geom_smooth(data = df_nm, aes(x = Year, y = mean_yield, 
                                  color = "NM"), method = "lm",
                    se = TRUE, linetype = "dashed",fill = "#5f5e62", 
               size = 1) +
    geom_smooth(data = df_tm, aes(x = Year, y = mean_yield, 
                                  color = "TM"), method = "lm",
                se = TRUE, linetype = "dashed", fill = "#2666fd", 
                size = 1) +
    annotate("text", x = max(df_nm$Year), y = 12000 - 1500, 
             label = paste("NM:", lm_eqn_nm, ", p = ", p_value_nm),
             family = font_name, hjust = 1, size = 3.5) +
    annotate("text", x = max(df_nm$Year), y = 12000 - 2200, 
             label = paste("TM:", lm_eqn_tm, ", p = ", p_value_tm),
             family = font_name, hjust = 1, size = 3.5) +
    labs(x = "Year", y = expression("GWP" ~ (kg * " " * CO[2] * " " * eq*" " *ha^-1))) +
    scale_x_continuous(limits = c(2020, 2100), 
                       breaks = seq(2020, 2100, by = 10), 
                       expand = c(0, 1.0)) +
    # 根据地区动态添加标注
    annotate("text", x = 2020, y = 11900, label = label, 
             hjust = 0, vjust = 1, family = font_name, 
             size = 5, fontface = "plain", color = "black") +
    scale_y_continuous(limits = c(000, 12000), 
                       breaks = seq(000, 12000, by = 3000), 
                       expand = c(0, 0.1)) +
    theme_minimal() +
  ...
  # 保存图形
  ggsave(paste0("126_", region, "_GWP.pdf"), plot, 
...
for (region in regions) {
  plot_yield_trends(region, "126-NM_GWP.xlsx", 
                    "126-TM_GWP.xlsx")
}
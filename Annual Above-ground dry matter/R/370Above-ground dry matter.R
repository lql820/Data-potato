...
plot_yield_trends <- function(region, nm_file, tm_file) {
  ...
  # 提取所需列
  df_nm <- df_nm %>% select(Year, mean_yield = ShootC_1_mean, 
                            sd_yield = ShootC_1_sd)
  df_tm <- df_tm %>% select(Year, mean_yield = ShootC_1_mean, 
                            sd_yield = ShootC_1_sd)
  ...
  plot <- ggplot() +
    ...
    geom_smooth(data = df_tm, aes(x = Year, y = mean_yield, 
                                  color = "TM"), method = "lm",
                se = FALSE, linetype = "dashed", size = 1) +
    annotate("text", x = max(df_nm$Year), y = 420, 
             label = paste("NM:", lm_eqn_nm, ", p = ", p_value_nm), family = font_name, hjust = 1, size = 3.5) +
    annotate("text", x = max(df_nm$Year), y = 300, 
             label = paste("TM:", lm_eqn_tm, ", p = ", p_value_tm), family = font_name, hjust = 1, size = 3.5) +
    labs(x = "Year", y = expression("Aboveground biomass" ~ (kg * " " * C * " " * ha^-1))) +
    scale_x_continuous(limits = c(2020, 2100), 
                       breaks = seq(2020, 2100, by = 10), 
                       expand = c(0, 1.0)) +
    # 根据地区动态添加标注
    annotate("text", x = 2020, y = 2080, label = label, 
             hjust = 0, vjust = 1, family = font_name, 
             size = 5, fontface = "plain", color = "black") +
    scale_y_continuous(limits = c(000, 2100), 
                       breaks = seq(000, 2100, by = 400), 
                       expand = c(0, 0.1)) +
    theme_minimal() +
  ...
  # 保存图形
  ggsave(paste0("126_", region, "_biomass.pdf"), plot, 
...
for (region in regions) {
  plot_yield_trends(region, "370-NM_ShootC_1_result.xlsx", 
                    "370-TM_ShootC_1_result.xlsx")
}